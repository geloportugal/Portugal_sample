#Initializing function blocks for reusability
def display_items(items):
    print("\nAvailable Items:")
    for idx in range(len(items)):
        print(f"{idx + 1}. {items[idx][0]} - PHP {items[idx][1]:.2f} ({items[idx][3]}% VAT) - Stock: {items[idx][2]}")

def calculate_vat(price, tax_rate):
    return price * (tax_rate / 100)

def generate_receipt(cart, items, payment):
    print("\nRECEIPT")
    total = 0
    print(f"{'Item':<20}{'Price':<10}{'VAT':<10}")
    for i in range(len(cart)):
        item_idx, qty = cart[i]
        item_name = items[item_idx][0]
        price = items[item_idx][1]
        tax_rate = items[item_idx][3]
        vat = calculate_vat(price, tax_rate) * qty
        subtotal = (price + vat) * qty
        total += subtotal
        print(f"{item_name:<20}PHP {price * qty:.2f} PHP {vat:.2f}")
...
items = [
    ["Rice", 40, 50, 0],
    ["Canned Tuna", 30, 20, 12],
    ["Milk", 50, 30, 12],
    ["Eggs (dozen)", 100, 10, 12],
    ["Medicine", 500, 10, 0],
    ["Shampoo", 150, 15, 12],
    ["Toothpaste", 80, 25, 12],
    ["Soap", 40, 40, 12],
    ["Cooking Oil", 70, 20, 12],
    ["Detergent", 90, 20, 12]
]
...
while True:
    ...
    item_name, price, stock, tax_rate = items[item_idx]
...
print("DISCLAIMER: Value-added tax will be applied after check-out.")
payment = 0
while True:
    total = 0
    for i in range(len(cart)):
        ...
        price = items[item_idx][1]
        tax_rate = items[item_idx][3]
        total += (price + calculate_vat(price, tax_rate)) * qty
